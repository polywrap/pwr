...
pub extern "C" fn _wrap_invoke(method_size: u32, args_size: u32, env_size: u32) -> bool {
    ...
    match args.method.as_str() {
        "buildManifest" => {
            result = build_manifest_wrapped(args.args.as_slice(), env_size);
        }
        "buildModule" => {
            result = build_module_wrapped(args.args.as_slice(), env_size);
        }
        "deploy" => {
            result = deploy_wrapped(args.args.as_slice(), env_size);
        }
        "buildAndDeploy" => {
            result = build_and_deploy_wrapped(args.args.as_slice(), env_size);
        }
        _ => {
